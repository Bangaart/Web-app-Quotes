...
- run command in docker from the folder where project located:
...
- Then there are two ways how to fill data:
...
- 2 - the second way is to run docker command inside container:

``` sh 
docker container exec web python manage.py migration_script
```
